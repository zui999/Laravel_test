  ...
  <div class="col-md-11 col-md-offset-1">
    <table class="table text-center">
      <tr>
        ...
        <th class="text-center">カートから外す</th>
      </tr>
      @foreach($shopcartitems as $shopcartitem)
      <tr>
        <td>{{ $shopcartitem->name }}</td>
        <td>{{ $shopcartitem->price }}</td>
        <td>{{ $shopcartitem->author }}</td>
        <td>
            <input type="text" name="numberToBuy" value="1" size="4">

        </td>
        <td>
          <form action="/deleteFromCart/{{ $shopcartitem->id }}" method="post">
